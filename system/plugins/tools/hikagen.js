...
module.exports = {
  ...
  async run(m, { sock, text, Func, config }) {
    ...
    try {
      ...
      const { data } = await axios.get(apiUrl.toString(), { responseType: "arraybuffer" });
...
async function createAnimatedSticker(m, prompt, style, config) {
  ...
  for (let i = 0; i <= words.length; i++) {
    ...
    const { data } = await axios.get(frameUrl.toString(), { responseType: "arraybuffer" });
...
async function sendSticker(m, buffer, config) {
  ...
  const sticker = await writeExif({ mimetype: metadata.mime, data: buffer }, { packName: config.sticker.packname, packPublish: config.sticker.author });
  m.reply({ sticker });
}