...
module.exports = (connection, store) => {
  ...
  function updateNameToDb(contacts) {
    ...
    for (const contact of contacts) {
      ...
      if (!chats) chats = sock.chats[id] = { id };
      sock.chats[id] = {
        ...chats,
        ...({
          ...contact,
  ...
  sock.ev.on(
    ...
    async function updateParticipantsToDb({ id, participants, action }) {
      id = sock.decodeJid(id);
      if (!(id in sock.chats)) sock.chats[id] = { id };
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      ...
      if (!(jid in sock.chats)) {
        sock.chats[jid] = { id: jid, messages: {} };
        } else if (!sock.chats[jid].messages) {
          sock.chats[jid].messages = {};
        }
        const messageID = msg.key.id;
        sock.chats[jid].messages[messageID] = msg;
        sock.chats[jid].isChats = true;

        if (!sock.chats[jid].name && msg.pushName) {
          sock.chats[jid].name = msg.pushName;
        }
      }
  });
  sock.ev.on(
  ...
  sock.ev.on("chats.upsert", async function chatsUpsertPushToDb(chatsUpsert) {
    ...
    if (isGroup) {
      ...
      const groups =
        (await sock.groupFetchAllParticipating().catch((_) => ({}))) || {};
  ...
  sock.ev.on(
    ...
    async function presenceUpdatePushToDb({ id, presences }) {
      ...
      if (!chats) chats = sock.chats[_sender] = { id: sender };
