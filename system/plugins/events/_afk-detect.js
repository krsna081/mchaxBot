      ...
      for (let jid of afk) {
         ...
         if (!m.fromMe) {
            m.reply(`Jangan tag @${jid.split('@')[0]}, Dia sedang AFK selama: *${Func.toTime(new Date - aefka)}*\n\n➠ *Reason:* ${reason ? reason : '-'}`).then(async () => {
               sock.sendMessage(jid, { text: `Seseorang dari grup *${await (await sock.groupMetadata(m.cht)).subject}*, menandai atau menyebut Anda.\n\n➠ *Pengirim:* @${m.sender.split('@')[0]}`, mentions: [m.sender] }, { quoted: m }).then(async () => {
