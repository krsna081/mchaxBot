...
Anda telah menerima ${users.warn}/5 peringatan.`}`);
    ...
    cron.schedule("* * * * *", async () => {
        const currentTime = moment().tz(config.tz).format("HH:mm:ss");
        ...
        for (const chatId in db.list().gcauto) {
            try {
                ...
                const closeReminderTime = moment(close + ":00", "HH:mm:ss").subtract(5, "minutes").format("HH:mm:ss");
                const openReminderTime = moment(open + ":00", "HH:mm:ss").subtract(5, "minutes").format("HH:mm:ss");
    ...
    cron.schedule("* * * * *", async () => {
        ...
        for (const chatId of Object.keys(store.groupMetadata)) {
            try {
                const bukaPuasa = "18:18:00";
                const reminderTime = moment(bukaPuasa, "HH:mm:ss").subtract(5, "minutes").format("HH:mm:ss");
